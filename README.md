...
- 半自动更新：程序可以读取公开的 `latest.json`，发现新版本后优先增量更新，失败时回退到完整安装包。
...
```text
https://github.com/Luofaiz/mentor-vault/releases/download/v0.2.20/latest.yml
```
...
`latest.yml` 和 `.blockmap` 是增量更新需要的文件；`MentorVaultSetup.exe` 和 `latest.json` 用于完整安装包兜底。
...
程序会先尝试增量更新。如果增量元数据不可用或下载失败，再按 `downloadUrls` 顺序下载完整安装包。
